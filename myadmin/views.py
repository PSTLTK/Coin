...
def NewMember(request):
    ...
    uc = UserChoiceModel.objects.filter(roundno=ran.roundno)
    if ran.total <= 13:
        bb = "bear"
    else:
        bb = "bull"
    ...
    if ran.total % 2 ==0:
        ...
        ts = "single"
    cw = UserChoiceModel.objects.filter(Q(choice=bb) | Q(choice=ts))  
    return render(request,'new_membertable.html',{"ran":ran,"uc":uc,"cw":cw})

def NMChR(request):
    ran = RandomModel.objects.get(status="now")
    if request.method == "POST":
        ran.a = request.POST.get('a')
        ran.b = request.POST.get('b')
        ran.c = request.POST.get('c')
        ran.total = request.POST.get('t')
        ran.save()
        return redirect('/myadmin/viewnewmember/')

def UserChDel(request):
    users = User.objects.all()
    for user in users:
        print("++++++++++++++++++",user)
        try:
            data = UserChoiceModel.objects.filter(user=user.id).order_by("-id")[5:]
        except:
            data = 0
...
def AdminRoomChoice(request):
    return render(request,'admin_room_choice.html')
